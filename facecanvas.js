...
function FaceCanvas(canvas, debugcanvas) {
    ...
    canvas.repaint = function() {
        ...
        if (canvas.active && canvas.animating) {
            ...
            let theta = 0;
            let epsilon = [expressions.sv[0]*Math.cos(theta),expressions.sv[1]*Math.sin(theta),0,0,0,0,0,0,0,0];


            //TODO: Use numeric.js to do the matrix multiplication and addition of the center
            //Unravel the array into a 2D array

            points1D = numeric.add(numeric.dot(expressions.PC,epsilon),expressions.center);
            ...
            for (let i = 0; i < points1D.length; i += 2) {
             ...
             points2D.push(textureShader.points[72]); 
             points2D.push(textureShader.points[73]);
             points2D.push(textureShader.points[74]);
             points2D.push(textureShader.points[75]);
